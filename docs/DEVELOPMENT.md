...
- `src/lib/` — folder-per-module (same pattern as components): each module lives in its own folder with the implementation, colocated test, and an `index.ts` barrel — imports resolve through the folder (`import { useLocale } from '$/lib/language'`). Modules: `language` (external store, `useSyncExternalStore`, `useLocale`/`setLanguage`, localStorage `matuz.dev:language`), `motion` (EASE, fadeUp, fade, staggerContainer, viewportOnce, pageVariants; default duration 1s), `notes` (client fetch + session cache), `relative-time`, `hash-scroller`, `page-meta` (`usePageMeta` — per-route tab title, meta description, OG/Twitter mirrors, optional `noindex`).
