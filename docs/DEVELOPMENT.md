...
- `src/lib/` — `language.ts` (external store, `useSyncExternalStore`, `useLocale`/`setLanguage`, localStorage `matuz.dev:language`), `motion.ts` (EASE, fadeUp, fade, staggerContainer, viewportOnce, pageVariants; default duration 1s), `notes.ts` (client fetch + session cache), `relative-time.ts`, `hash-scroller.tsx`, `page-meta.ts` (`usePageMeta` — per-route tab title, meta description, OG/Twitter mirrors, optional `noindex`).
