...
const indexCache: { entry: CacheEntry<NoteIndexEntry[]> | null } = { entry: null };
...
async function fetchFile(
    ...
    return { raw: Buffer.from(payload.content, 'base64').toString('utf-8'), etag: newEtag };
}
...
export async function getNoteContent(
    ...
    contentCaches.set(slug, { value: note, etag: result.etag, fetchedAt: Date.now() });
